...
async function loadConfig() {
    const configPath = path.join(process.env.HOME || "~", ".brain-mcp", "config.json");
    const raw = await fs.readFile(configPath, "utf-8");
    return JSON.parse(raw);
}
function resolvePath(vaultPath, notePath) {
    const expanded = vaultPath.replace("~", process.env.HOME || "~");
    return path.join(expanded, notePath.endsWith(".md") ? notePath : `${notePath}.md`);
}
function resolveVaultPath(config, vault) {
    const key = vault || config.default;
    const vaultPath = config.vaults[key];
    if (!vaultPath)
        throw new Error(`Vault "${key}" not found in config. Available: ${Object.keys(config.vaults).join(", ")}`);
    return vaultPath;
}
// ─── Server ───────────────────────────────────────────────────────────────────
const server = new Server({ name: "brain-mcp", version: "1.0.0" }, { capabilities: { tools: {} } });
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        // ── read_note ──────────────────────────────────────────────────────────
        if (name === "read_note") {
            const vaultPath = resolveVaultPath(config, args.vault);
            const fullPath = resolvePath(vaultPath, args.path);
        ...
        if (name === "list_notes") {
            const vaultPath = resolveVaultPath(config, args.vault);
            const expanded = vaultPath.replace("~", process.env.HOME || "~");
            const folderPath = path.join(expanded, args.folder);
...
## Files touched
${args.files_touched ? args.files_touched.split(",").map((f) => `- ${f.trim()}`).join("\n") : "—"}
`;
            await fs.mkdir(path.dirname(fullPath), { recursive: true });
